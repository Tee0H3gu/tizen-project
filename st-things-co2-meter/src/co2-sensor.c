...
#define CO2_SENSOR_DEFAULT_VALUE_ZP (950) // 400ppm
#define CO2_SENSOR_DEFAULT_VALUE_1000 (400)
...
int resource_write_buzer(int pin_num, int write_value)
{
	...
	_I("Buzer Value : %s", write_value ? "ON":"OFF");

	return 0;
}

void resource_close_buzer(void)
{
	if (!g_sensor_h) return;

	_I("Buzer is finishing...");
...
unsigned int co2_sensor_voltage_to_ppm(double voltage)
{
	...
	if(ppm > 1000){
		resource_write_buzer(14, 1);
	}
	else{
		resource_write_buzer(14, 0);
	}
